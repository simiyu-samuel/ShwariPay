...
class ShwaripayValidationModuleFrontController extends ModuleFrontController
{
    public function postProcess()
    {
        ...
        $passkey = Configuration::get('M_PESA_PASS_KEY');
        $timestamp = date('YmdHis');
        $password = base64_encode($businessShortCode . $passkey . $timestamp);

        $callbackUrl = $this->context->link->getModuleLink('shwaripay', 'callback', '', true);
        // 'https://shwarimatt.com/module/shwaripay/callback'
        $requestPayload = [
        ...
        if (isset($response['ResponseCode']) && $response['ResponseCode'] === '0') {
            ...
            $url = $this->context->link->getModuleLink('shwaripay', 'payment', [
                'ttl' => $total,
                'stk_err' => $cart->id,
    ...
    private function getAccessToken()
    {
        ...
        $env = Configuration::get('M_PESA_LIVE_MODE');
